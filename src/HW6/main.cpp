...
const char *APP_NAME = "Raymarching";
int main()
{
    ...
    // -------------------------
    const std::string SRC = "../src/HW6/";
    Shader shader(SRC + "raymarching.vs.glsl", SRC + "raymarching.fs.glsl");
    shader.use();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (gui)
        {
            ...
            // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
            {
                ...
                ImGui::SliderFloat("progress", &progress, 0.0, 1.0); // Edit 1 float using a slider from 0.0f to 1.0f
...
void processInput(GLFWwindow *window)
{
    ...
    float moveDelta = deltaTime * (1 + glfwGetKey(window, GLFW_KEY_LEFT_SHIFT));
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
        camera.ProcessKeyboard(FORWARD, moveDelta);
    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
        camera.ProcessKeyboard(BACKWARD, moveDelta);
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
        camera.ProcessKeyboard(LEFT, moveDelta);
    if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
        camera.ProcessKeyboard(RIGHT, moveDelta);
    if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS)
        camera.ProcessKeyboard(UP, moveDelta);
    if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS)
        camera.ProcessKeyboard(DOWN, moveDelta);
}
